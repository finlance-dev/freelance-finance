...
function postLink(message, link) {
...
async function main() {
  ...
  // 3 posts per day: offset by time slot
  let slot = 0;
  if (hour >= 10) slot = 1;
  if (hour >= 14) slot = 2;
  ...
  try {
    const result = await postLink(message, link);
